...
const {DECK_OF_CARDS} = require('./utility-functions/card-functions');
		...
		switch (command) {
			...
			case 'roll':
				...
				if (args[0].toLowerCase().startsWith('r')) {
					...
					let autoSuccesses = optionalArguments.find((s) => s.match(/^-a\d/))
						? Number(
								optionalArguments
									.find((s) => s.match(/^-a\d/))
									.replace('-', '')
					...
					random
						...
						.then((result) => {
							...
							returnedNumbers.forEach((num, i) => {
								...
								if (num === 1) {
									...
									message.channel
										...
										.catch((error) => {
											log(
												'failed to add reaction to critical success message. See error: ',
												error
											);
										});
								}
								if (num === 100) {
									message.channel
										.send(
											`<:rip:752693741440991323> Wah wah wah. ${userAlias} failed catastrophically! (You rolled a \`${num}\`.)`
										)
										.then((sadMessage) => {
											sadMessage.react('😢');
										})
										.catch((error) => {
											log(
												'failed to add reaction to critical failure message. See error: ',
												error
											);
										});
								}
							...
							message.channel
								.send(`${userAlias} rolled... ${rolledNumberString} Result: ${resultString}`)
								.then((resultMessage) => {
					...
					if (
						/[0-9]{1,2}d[0-9]{1,3}/.test(args[0]) === false &&
						/\d{1,2}d\d[+-]\d{1,2}/.test(args[0]) === false
					) {
					...
					if (numOfDice === 0) {
						if (modifier) {
							replyToUserWithoutMention(
								message,
								`${userAlias} Roll: \`[0]\` + \`${modifier}\` \nTotal: \`${_.sum([0, modifier])}\``
							);
						} else {
						...
						.then((result) => {
							...
							if (messageContent.length > 2000) {
								let appendToEndString =
									'...`]' + messageContent.slice(messageContent.indexOf(']`') + 2);
			...
			case 'gurps':
				...
				random
					.generateIntegers(gurpsRandomConfig)
					.then((result) => {
						let returnedNumbers = result.random.data;
			...
			case 'motw':
				...
				random
					.generateIntegers(motwRandomConfig)
					.then((result) => {
						let returnedNumbers = result.random.data;
						let total = _.sum(returnedNumbers);
			...
			case 'fate':
				...
				random
					.generateIntegers(fateRandomConfig)
					.then((result) => {
						let returnedNumbers = result.random.data;
						...
						fateMappedNumbers.forEach((num) => {
							switch (num) {
								...
								default:
									log(
										`something went wrong in the fate dice conversion. [${num}] was not 0, 1, or -1)`
									);
							}
						});
			...
			case 'raccoon':
				...
				random
					.generateIntegers(raccoonRandomConfig)
					.then((result) => {
						let returnedNumbers = result.random.data;
			...
			case 'misty':
				{
					...
					random
						.generateIntegers(mistbornRandomConfig)
						.then((result) => {
							let returnedNumbers = result.random.data;
			...
			case 'draw':
				{
					...
					random
						.generateIntegerSequences(drawCardsConfig)
						.then((result) => {
